...
(function() {
    ...
    // Scroll modal incrementally to load new content
    async function scrollModalIncremental(modal) {
        ...
        // CRITICAL FIX: Do NOT reset scrollTop to 0
        // Resetting causes Facebook's virtual DOM to unload comments at the bottom,
        // which is a primary reason for missing comments.
    ...
    async function expandAllReplies(modal, maxComments) {
        ...
        while (iteration < maxIterations) {
            ...
            const replyPatterns = [
                ...
                /\d+\s+r[eé]ponse/i,
                // Spanish
                /ver\s+\d+\s+respuestas/i,
                /ver m[aá]s respuestas/i,
                /\d+\s+respuesta/i,
                // Portuguese
    ...
    async function expandMoreComments(modal) {
        ...
        const viewMoreCommentPatterns = [
            ...
            // Spanish
            /ver m[aá]s comentarios/i,
            /ver\s+\d+\s+comentarios/i,
            // Portuguese
            /ver mais coment[aá]rios/i,
            // Italian
    ...
    async function expandAllInModal(modal, maxComments) {
        ...
        while (cycle < maxCycles) {
            ...
            if (!activityThisCycle) {
                ...
                console.log(`⚠️ No activity this cycle (${consecutiveEmptyCycles}/5)`);

                if (consecutiveEmptyCycles >= 5) {
                    console.log('✅ No more content to load (5 empty cycles)');
                    break;
                }
            } else {
        ...
        for (let articleIndex = 0; articleIndex < articles.length; articleIndex++) {
            ...
            if (isCommentArticle(article)) {
                ...
                if (comment && comment.text && comment.text.length > 0) {
                    ...
                    console.log(`⚠️ Skipped article (no valid comment data) - aria-label: ${ariaLabel ? ariaLabel.substring(0, 50) : '[none]'}`);
                }
            }
        }
    ...
    async function startScraping() {
        ...
        try {
            ...
            while (true) {
                ...
                updateUI('🔍 Scanning...', {
                    statusText: `Collected ${allCollectedComments.length}`,
                    mainComments: stats.mainComments,
                    replies: stats.replies,
                    scrapedCount: allCollectedComments.length,
                    maxLimit: maxComments,
                    showStats: true
                });
